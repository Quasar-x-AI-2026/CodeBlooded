...
export default function IndiaMap() {
  ...
  const getColor = (risk) => {
    if (risk > 80) return "#003362  "; // soft red
    if (risk > 60) return "#003362  "; // soft amber
    if (risk > 40) return "#003362  "; // soft green
    return "#003362"; // very light green
  };
    ...
    >
      ...
      <div className="h-[800px] w-[900px] relative">
        ...
        >
          ...
          {["disaster", "crisis", "ngo"].map(
  (type) =>
    activeLayers[type] && (
      <LayerGroup key={type}>
        {MAP_MARKERS.filter((m) => m.type === type).map((m) => (
          <Marker
            key={m.id}
